// ─────────────────────────────────────────────────────────────────────────────
//  QCM TP5 — Fonctions · Tuples · Dictionnaires · Exceptions
//  PCEP Module 4  |  ISET Béja  |  60s strict par question
...
// ═══════════════════════════════════════════════════════════════════════════
// QUESTIONS — 10 questions PCEP Module 4
// ═══════════════════════════════════════════════════════════════════════════
const QUESTIONS = [
  // ── Niveau 1 : Fonctions de base ─────────────────────────────────────────
  {
    id: 1, niveau: 1, theme: "def / return — valeur retournée",
    code: `def carre(n):
    n * n

print(carre(5))`,
    question: "Quelle est la sortie ?",
    options: ["25", "None", "0", "SyntaxError"],
    correct: 1,
    explication: [
      "La fonction calcule n*n mais n'a pas de return.",
      "Sans return, une fonction retourne None implicitement.",
      "print(None) affiche : None",
      "→ Correction : ajouter  return n * n",
    ],
  },
  {
    id: 2, niveau: 1, theme: "Paramètres par défaut",
...
saluer("Chorfi", "Mme.")`,
    ...
    options: [
      "Bonjour Ben Ali · Bonjour Chorfi",
      "Bonjour M. Ben Ali · Bonjour Mme. Chorfi",
      "Bonjour M. Ben Ali · Bonjour M. Chorfi",
      "SyntaxError",
    ],
    correct: 1,
    explication: [
      "Premier appel : titre prend sa valeur par défaut = \"M.\"",
      "→ Bonjour M. Ben Ali",
      "Deuxième appel : titre = \"Mme.\" (valeur passée)",
      "→ Bonjour Mme. Chorfi",
    ],
  },
  {
    id: 3, niveau: 1, theme: "Variable locale vs globale",
    code: `x = 10
...
print(x)`,
    question: "Quelles sont les deux sorties (dans l'ordre) ?",
    options: ["99 · 99", "10 · 10", "99 · 10", "10 · 99"],
    correct: 2,
    explication: [
      "Dans f(), x = 99 crée une variable LOCALE à f.",
      "La variable globale x = 10 reste inchangée.",
      "f()      → affiche 99  (variable locale)",
      "print(x) → affiche 10  (variable globale inchangée)",
    ],
  },
  // ── Niveau 2 : Tuples & Dictionnaires ────────────────────────────────────
  {
    id: 4, niveau: 2, theme: "Tuple — immuabilité",
    code: `t = (1, 2, 3)
t[0] = 99
print(t)`,
    ...
    correct: 2,
    explication: [
      "Les tuples sont IMMUABLES en Python.",
      "t[0] = 99 tente de modifier un tuple → TypeError.",
      "Message : 'tuple' object does not support item assignment",
      "Pour modifier : convertir en liste avec list(t).",
    ],
  },
  {
    id: 5, niveau: 2, theme: "Dictionnaire — accès et ajout",
    code: `d = {"nom": "Ali", "age": 20}
d["email"] = "ali@iset.tn"
print(len(d))
print(d.get("prenom", "inconnu"))`,
    question: "Quelles sont les deux sorties (dans l'ordre) ?",
    options: ["2 · None", "3 · inconnu", "2 · inconnu", "3 · None"],
    correct: 1,
    explication: [
      "d commence avec 2 clés : nom et age.",
      "d[\"email\"] = ... ajoute une 3ème clé → len(d) = 3",
      "d.get(\"prenom\", \"inconnu\") : prenom absent → retourne \"inconnu\"",
      "→ Sorties : 3 puis inconnu",
    ],
  },
  {
    id: 6, niveau: 2, theme: "Dictionnaire — items() et boucle",
    code: `scores = {"Ali": 15, "Sara": 18, "Slim": 12}
for nom, note in scores.items():
    if note >= 15:
        print(nom)`,
    question: "Quelle est la sortie ?",
    options: ["Ali · Sara · Slim", "Ali · Sara", "Sara", "15 · 18"],
    correct: 1,
    explication: [
      "scores.items() retourne les paires (clé, valeur).",
      "Ali  : note=15 → 15>=15 True  → affiché",
      "Sara : note=18 → 18>=15 True  → affichée",
      "Slim : note=12 → 12>=15 False → non affiché",
      "→ Sortie : Ali puis Sara",
    ],
  },
  // ── Niveau 2 : Récursion ──────────────────────────────────────────────────
  {
    id: 7, niveau: 2, theme: "Récursivité — factorielle",
    code: `def fact(n):
    if n == 0:
        return 1
    return n * fact(n - 1)
...
print(fact(4))`,
  ...
  // ── Niveau 3 : Exceptions ────────────────────────────────────────────────
  {
    id: 8, niveau: 3, theme: "try / except — ValueError",
    code: `try:
    n = int("abc")
    print("OK :", n)
except ValueError:
    ...
    correct: 2,
    explication: [
      "int(\"abc\") lève une ValueError.",
      "Le bloc except ValueError: s'exécute → affiche 'Invalide'",
      "Le bloc finally: s'exécute TOUJOURS → affiche 'Fin'",
      "→ Sortie : Invalide puis Fin",
    ],
  },
  {
    id: 9, niveau: 3, theme: "except — KeyError dictionnaire",
    code: `carnet = {"Ali": "20 123 456"}
...
try:
    ...
    print("Erreur :", e)`,
    question: "Quelle est la sortie ?",
    options: ["None", "Erreur : Sara", "Introuvable", "20 123 456"],
    correct: 2,
    explication: [
      "carnet[\"Sara\"] : la clé Sara n'existe pas → KeyError.",
      "except KeyError: est testé EN PREMIER → s'exécute.",
      "except Exception: ne sera JAMAIS atteint ici.",
      "→ Sortie : Introuvable",
      "Règle : du plus spécifique au plus général.",
    ],
  },
  {
    id: 10, niveau: 3, theme: "Synthèse — global + dict + except",
    code: `compteur = 0

def incrementer(dico, cle):
    global compteur
    try:
        dico[cle] += 1
        compteur += 1
...
print(compteur)`,
    ...
    correct: 0,
    explication: [
      "Appel 1 : d[\"a\"] absent → KeyError → d={\"a\":1}, compteur=1",
      "Appel 2 : d[\"a\"]=1 existe → d[\"a\"]+=1 → d={\"a\":2}, compteur=2",
      "Appel 3 : d[\"b\"] absent → KeyError → d={\"a\":2,\"b\":1}, compteur=3",
      "→ d = {'a': 2, 'b': 1}   compteur = 3",
    ],
  },
];
...
const DB_KEY      = "qcm_tp5";
const TIME_LIMIT  = 60;

const NIV = {
  1: { bg:"#E8F5E9", border:"#27AE60", color:"#1E7E4C", label:"Niveau 1 — Fonctions" },
  2: { bg:"#E3F2FD", border:"#2E86C1", color:"#1A5276", label:"Niveau 2 — Tuples & Dicts" },
  3: { bg:"#FFF3E0", border:"#E67E22", color:"#B07A00", label:"Niveau 3 — Exceptions & Synthèse" },
};
...
function TeacherDashboard({ onBack }) {
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0D1F2D",fontFamily:F,color:"#E8F4FB"}}>
      <div style={{background:"#0F2A45",borderBottom:"3px solid #9B59B6",padding:"14px 22px",
                   display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
        <div>
          <h1 style={{margin:0,fontSize:19,fontWeight:800,color:"#9B59B6"}}>
            📊 Tableau de bord — TP5 Module 4
          </h1>
          <p style={{margin:0,fontSize:11,color:"#5B7A9A"}}>
            Fonctions · Tuples · Dictionnaires · Exceptions · Temps réel Firebase
          </p>
        </div>
      ...
      <div style={{padding:"18px 22px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(148px,1fr))",gap:12,marginBottom:20}}>
          {[
            ...
            {label:"Réussite",    value:avgPct+"%",  sub:"moyenne",       color:"#9B59B6",icon:"🎯"},
            {label:"Terminé",     value:finished,    sub:"étudiants",     color:"#E67E22",icon:"✅"},
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
          ...
          <div style={{background:"#0F2A45",borderRadius:11,padding:16,border:"1px solid #1A4A7A"}}>
            <h3 style={{margin:"0 0 12px",color:"#9B59B6",fontSize:14}}>🏆 Classement en direct</h3>
                  ...
                  return (
                    <div key={r.name} style={{display:"flex",alignItems:"center",gap:7,
                      ...
                      <span style={{fontSize:12,fontWeight:700,
                                    color:pct>=70?"#2ECC71":pct>=50?"#9B59B6":"#E74C3C"}}>
                        {r.score}/{r.total}
                      </span>
                      <div style={{width:52}}>
                        <div style={{background:"#1A3A5A",borderRadius:3,height:5,overflow:"hidden"}}>
                          <div style={{width:`${pct}%`,height:"100%",
                                       background:pct>=70?"#2ECC71":pct>=50?"#9B59B6":"#E74C3C",
                ...
                <div key={q.id} onClick={()=>setSelQ(selQ?.id===q.id?null:q)}
                  ...
                  {selQ?.id===q.id&&(
                    <div style={{marginTop:7,background:"#0A1A2A",borderRadius:7,padding:9}}>
                      <p style={{margin:"0 0 5px",fontSize:10,color:"#9B59B6",fontWeight:700}}>
                        {q.question}
                      </p>
...
function StudentQCM({ studentName }) {
  ...
  useEffect(()=>{
    ...
    setShowExp(false); setTimer(TIME_LIMIT); setTimedOut(false);
  },[current]);

  useEffect(()=>{
    if (confirmed||finished) return;
    if (timer<=0) {
      setTimedOut(true); setConfirmed(true);
      setAnswers(p=>[...p,{qid:q.id,chosen:null,correct:false,time:TIME_LIMIT,timedOut:true}]);
      return;
    }
  ...
  const handleConfirm=()=>{
    ...
    setConfirmed(true);
    setAnswers(p=>[...p,{qid:q.id,chosen:selected,correct:selected===q.correct,
  ...
  const tBord  = timer<=10?"#E74C3C":timer<=20?"#E67E22":"#DDE7F0";
  const barCol = timer<=10?"#E74C3C":timer<=20?"#E67E22":"#9B59B6";
  const F="'Trebuchet MS',sans-serif";
    ...
    return (
      <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#EAF4FB,#F4F9FD)",
        ...
        <div style={{background:"#fff",borderRadius:18,padding:"32px 40px",maxWidth:500,
                     width:"100%",boxShadow:"0 8px 40px #6C348320",textAlign:"center"}}>
          ...
          <p style={{color:"#5B7A9A",marginBottom:20,fontSize:13}}>
            TP5 — Fonctions · Tuples · Dictionnaires · Exceptions
          </p>
          <div style={{display:"flex",justifyContent:"center",gap:12,marginBottom:18,flexWrap:"wrap"}}>
            {[
              ...
              {val:`${pct}%`,label:"Score",bg:pct>=80?"#E8F5E9":pct>=60?"#F3E5F5":"#FDECEA",
               col:pct>=80?"#1E7E4C":pct>=60?"#6C3483":"#C0392B"},
              {val:`${toCount}`,label:"Temps écoulés",bg:toCount>0?"#FDECEA":"#E8F5E9",
               col:toCount>0?"#C0392B":"#1E7E4C"},
            ].map((k,i)=>(
          ...
          {!submitted?(
            <button onClick={handleSubmit} disabled={submitting}
              style={{background:submitting?"#AAA":"#6C3483",color:"#fff",border:"none",
      ...
      <div style={{maxWidth:700,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",
          ...
          <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
            ...
            <span style={{background:tBg,color:tColor,border:`2px solid ${tBord}`,
        ...
        {!confirmed&&(
          <div style={{background:"#DDE7F0",borderRadius:6,height:7,marginBottom:6,overflow:"hidden"}}>
            <div style={{height:"100%",width:`${timer/TIME_LIMIT*100}%`,
        ...
        <div style={{background:"#EEF4F8",borderRadius:6,height:4,marginBottom:14,overflow:"hidden"}}>
          <div style={{height:"100%",width:`${progress}%`,background:"#9B59B6",transition:"width 0.4s"}}/>
        </div>
        ...
        {/* Card */}
        <div style={{background:"#fff",borderRadius:13,boxShadow:"0 4px 20px #6C348310",
                     padding:"20px 22px",marginBottom:12}}>
          <CodeBlock code={q.code}/>
          <p style={{fontSize:15,fontWeight:700,color:"#1A2E40",marginBottom:14}}>
            {q.question}
          </p>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            {q.options.map((opt,i)=>{
              let bg=selected===i?"#F3E5F5":"#F8FBFE";
              let border=selected===i?"2px solid #6C3483":"2px solid #DDE7F0";
              let color=selected===i?"#6C3483":"#1A2E40";
              let bbg=selected===i?"#6C3483":"#DDE7F0";
          ...
          {confirmed&&(
            <div style={{marginTop:12}}>
              <div style={{background:timedOut?"#FFF3E0":isCorrect?"#E8F5E9":"#FDECEA",
                ...
                <div>
                  <div style={{fontWeight:700,
                               ...
                               fontSize:13}}>
                    {timedOut?"Temps écoulé ! Passage automatique."
              ...
              <button onClick={()=>setShowExp(v=>!v)}
                style={{background:"none",border:"1px solid #6C3483",color:"#6C3483",
                ...
                <div style={{marginTop:8,background:"#F8FBFE",border:"1px solid #DDE7F0",
                  ...
                  {q.explication.map((l,i)=>(
                    <p key={i} style={{margin:"2px 0",fontSize:11.5,color:"#1A2E40",
                      fontFamily:l.startsWith("→")||l.includes("=")||l.includes("def")||
                                 l.includes("return")||l.includes("except")||
                                 l.includes("global")||l.includes("fact")
                                 ?"'Courier New'":"inherit"}}>
                      {l}
                    </p>
                  ))}
        ...
        <div style={{display:"flex",justifyContent:"flex-end"}}>
          {!confirmed?(
            <button onClick={handleConfirm} disabled={selected===null}
              style={{background:selected!==null?"#6C3483":"#DDE7F0",
                      color:selected!==null?"#fff":"#5B7A9A",border:"none",
    ...
    <div style={{minHeight:"100vh",
      ...
      <div style={{width:"100%",maxWidth:440}}>
        <div style={{textAlign:"center",marginBottom:28}}>
          ...
          <h1 style={{fontSize:26,fontWeight:900,color:"#9B59B6",margin:"0 0 4px"}}>QCM TP5</h1>
          <p style={{color:"#5B7A9A",fontSize:13,margin:"0 0 8px"}}>
            PCEP — Module 4 · Fonctions · Tuples · Dicts · Exceptions
          </p>
          ...
          {!tMode?(
            <>
              ...
              <p style={{color:"#5B7A9A",fontSize:11,marginBottom:18}}>
                {QUESTIONS.length} questions · 3 niveaux · 60 secondes par question
              </p>
              ...
              <button onClick={()=>onStudent(name.trim())} disabled={name.trim().length<3}
                style={{width:"100%",
                        background:name.trim().length>=3
                          ?"linear-gradient(90deg,#6C3483,#9B59B6)":"#1A3A5A",
            ...
            <>
              <h2 style={{margin:"0 0 4px",color:"#9B59B6",fontSize:16}}>🔐 Accès enseignant</h2>
