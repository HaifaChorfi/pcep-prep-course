...
const QUESTIONS = [
  {
    ...
    correct: 1,
    explication: `sep="" supprime l'espace par défaut → ABC\nend="!" remplace \\n final → ABC!`,
  },
  {
  ...
  },
  {
    ...
    correct: 2,
    explication: `print() sans argument affiche une ligne vide.\nC'est le \\n de end= par défaut qui est affiché.`,
  },
  {
    ...
    correct: 1,
    explication: `0b101 = 5 (binaire)\n0o7 = 7 (octal)\n0xA = 10 (hexadécimal)\nPython convertit tout en décimal pour l'affichage.`,
  },
  {
  ...
  },
  {
    ...
    correct: 1,
    explication: `"3" et "14" sont des str.\n+ entre deux str = concaténation, pas addition.\nx = "314" (str), pas 17 (int).`,
  },
  {
    ...
    correct: 1,
    explication: `bool hérite de int en Python.\nTrue = 1, False = 0\n1 + 1 + 0 = 2`,
  },
  {
  ...
  },
  {
    ...
    correct: 1,
    explication: `Python est sensible à la casse.\nPrint (majuscule) ≠ print → NameError.\nprint est la seule forme valide.`,
  },
  {
    ...
    options: ["42\nFin", "Fin", "42", "42 Fin"],
    correct: 1,
    explication: `En mode script, les expressions sans print() ne s'affichent pas.\n40+2 est calculé mais non affiché.\nSeul print("Fin") produit une sortie → Fin.`,
  },
];
...
const TEACHER_PWD = "iset2025"; // ← Changer ce mot de passe !
  ...
  return (
    <div style={{
      ...
      {code.split("\n").map((line, i) => {
        ...
        const parts = [];
        if (isComment) {
          parts.push(<span key="c" style={{ color: "#4A7A9A", fontStyle: "italic" }}>{line}</span>);
        } else {
          const strRe = /("(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*')/g;
          let last = 0, m;
...
function TeacherDashboard({ onBack }) {
  ...
  useEffect(() => {
    const unsubscribe = onValue(ref(db, "qcm_tp1"), (snapshot) => {
  ...
  const avgScore = total
    ...
    : 0;
  const avgPct = total
    ? Math.round((results.reduce((s, r) => s + r.score / r.total, 0) / total) * 100)
    : 0;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0D1F2D", fontFamily: "'Trebuchet MS', sans-serif", color: "#E8F4FB" }}>
      {/* Header */}
      <div style={{ background: "#0F2A45", borderBottom: "3px solid #F0A500", padding: "16px 28px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 22, fontWeight: 800, color: "#F0A500" }}>📊 Tableau de bord — Enseignant</h1>
          <p style={{ margin: 0, fontSize: 13, color: "#5B7A9A" }}>
            QCM TP1 · Python Essentials 1 · Mise à jour en temps réel (Firebase)
          </p>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <button
            onClick={async () => {
              if (window.confirm("Effacer tous les résultats de la session ?")) {
      ...
      <div style={{ padding: "24px 28px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 16, marginBottom: 28 }}>
          {[
            ...
            { label: "Score moyen", value: `${avgScore} / ${QUESTIONS.length}`, sub: "bonnes réponses", color: "#2ECC71", icon: "📈" },
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          {/* Classement */}
          <div style={{ background: "#0F2A45", borderRadius: 14, padding: 20, border: "1px solid #1A4A7A" }}>
            <h3 style={{ margin: "0 0 16px", color: "#F0A500", fontSize: 16 }}>🏆 Classement en direct</h3>
            {loading ? (
              <p style={{ color: "#5B7A9A" }}>Connexion Firebase…</p>
            ) : results.length === 0 ? (
              <p style={{ color: "#5B7A9A", fontSize: 13 }}>
                Aucun résultat pour l'instant.<br />
                Les étudiants doivent soumettre leur QCM.
              </p>
            ) : (
              ...
              {qStats.map(({ q, answered, correct, pct }) => (
                <div key={q.id} onClick={() => setSelectedQ(selectedQ?.id === q.id ? null : q)}
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                    <span style={{ background: NIVEAU_STYLE[q.niveau].border + "33", color: NIVEAU_STYLE[q.niveau].border, borderRadius: 4, padding: "1px 6px", fontSize: 10, fontWeight: 700 }}>
                      N{q.niveau}
                    </span>
                    <span style={{ flex: 1, fontSize: 13, color: "#8AACBF" }}>Q{q.id} — {q.theme}</span>
                    <span style={{ fontSize: 12, color: "#5B7A9A" }}>{answered} rép.</span>
                    {pct !== null && (
                      ...
                      {q.options.map((opt, oi) => {
                        ...
                        return (
                          <div key={oi} style={{ display: "flex", alignItems: "center", gap: 6, margin: "3px 0" }}>
                            <span style={{ fontSize: 11, color: oi === q.correct ? "#2ECC71" : "#5B7A9A", minWidth: 14 }}>
                              {oi === q.correct ? "✓" : "·"}
                            </span>
                            <span style={{ fontSize: 11, color: oi === q.correct ? "#2ECC71" : "#5B7A9A", flex: 1 }}>{opt}</span>
                            <span style={{ fontSize: 11, color: "#5B7A9A" }}>
                              {chosenBy.length} étudiant{chosenBy.length > 1 ? "s" : ""}
                            </span>
                          </div>
                        );
                      })}
...
function StudentQCM({ studentName }) {
  ...
  const handleConfirm = () => {
    ...
    setConfirmed(true);
    setAnswers((prev) => [...prev, { qid: q.id, chosen: selected, correct: selected === q.correct, time: timer }]);
  };
      ...
      <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#EAF4FB,#F4F9FD)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Trebuchet MS', sans-serif", padding: 24 }}>
        <div style={{ background: "#fff", borderRadius: 20, padding: "40px 48px", maxWidth: 520, width: "100%", boxShadow: "0 8px 40px #1A6FA822", textAlign: "center" }}>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 20, marginBottom: 28 }}>
            ...
            <div style={{ background: "#F3E5F5", borderRadius: 12, padding: "16px 20px" }}>
              <div style={{ fontSize: 36, fontWeight: 800, color: "#7B1FA2" }}>
                {Math.round(answers.reduce((s, a) => s + a.time, 0) / answers.length)}s
              </div>
              <div style={{ fontSize: 12, color: "#5B7A9A" }}>Temps moyen</div>
            </div>
          </div>
          ...
          {!submitted ? (
            <button onClick={handleSubmit} disabled={submitting}
              style={{ background: submitting ? "#AAA" : "#1A6FA8", color: "#fff", border: "none", borderRadius: 10, padding: "14px 36px", fontSize: 16, fontWeight: 700, cursor: submitting ? "default" : "pointer", width: "100%" }}>
              {submitting ? "⏳ Envoi en cours…" : "📤 Envoyer mes résultats à l'enseignant"}
            </button>
          ) : (
            <div style={{ background: "#E8F5E9", border: "1px solid #2ECC71", borderRadius: 10, padding: 16, color: "#1E7E4C", fontWeight: 700 }}>
              ✅ Résultats envoyés avec succès !<br />
              <span style={{ fontWeight: 400, fontSize: 13 }}>L'enseignant peut voir votre score en temps réel.</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <span style={{ background: timer > 30 ? "#FDECEA" : "#F8FBFE", color: timer > 30 ? "#C0392B" : "#5B7A9A", border: `1px solid ${timer > 30 ? "#E74C3C" : "#DDE7F0"}`, borderRadius: 8, padding: "4px 10px", fontSize: 13, fontWeight: 700 }}>
              ⏱ {timer}s
            </span>
            {answers.length > 0 && (
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
          <span style={{ background: niv.bg, color: niv.color, border: `1px solid ${niv.border}`, borderRadius: 6, padding: "3px 10px", fontSize: 12, fontWeight: 700 }}>
            {niv.label}
          </span>
          <span style={{ background: "#F8FBFE", color: "#5B7A9A", border: "1px solid #DDE7F0", borderRadius: 6, padding: "3px 10px", fontSize: 12 }}>
            {q.theme}
          </span>
        </div>
        ...
        <div style={{ background: "#fff", borderRadius: 16, boxShadow: "0 4px 24px #1A6FA812", padding: "24px 28px", marginBottom: 16 }}>
            ...
            {q.options.map((opt, i) => {
              ...
              return (
                <button key={i} onClick={() => !confirmed && setSelected(i)}
                  style={{ background: bg, border, borderRadius: 10, padding: "11px 14px", textAlign: "left", cursor: confirmed ? "default" : "pointer", display: "flex", alignItems: "center", gap: 10, transition: "all 0.2s" }}>
          ...
          {confirmed && (
            <div style={{ marginTop: 16 }}>
              <div style={{ background: isCorrect ? "#E8F5E9" : "#FDECEA", border: `1px solid ${isCorrect ? "#2ECC71" : "#E74C3C"}`, borderRadius: 10, padding: "10px 14px", display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                <span style={{ fontSize: 20 }}>{isCorrect ? "🎉" : "😅"}</span>
                <span style={{ fontWeight: 700, color: isCorrect ? "#1E7E4C" : "#C0392B", fontSize: 14 }}>
                  {isCorrect ? "Bonne réponse !" : `Incorrect — Réponse correcte : ${q.options[q.correct]}`}
                </span>
              </div>
              <button onClick={() => setShowExp((v) => !v)}
                style={{ background: "none", border: "1px solid #1A6FA8", color: "#1A6FA8", borderRadius: 8, padding: "5px 12px", fontSize: 12, cursor: "pointer", fontWeight: 600 }}>
              ...
              {showExp && (
                <div style={{ marginTop: 10, background: "#F8FBFE", border: "1px solid #DDE7F0", borderRadius: 10, padding: "12px 14px" }}>
                  {q.explication.split("\n").map((l, i) => (
                    <p key={i} style={{ margin: "3px 0", fontSize: 13, color: "#1A2E40" }}>{l}</p>
                  ))}
                </div>
              )}
            </div>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end" }}>
          {!confirmed ? (
            <button onClick={handleConfirm} disabled={selected === null}
              style={{ background: selected !== null ? "#1A6FA8" : "#DDE7F0", color: selected !== null ? "#fff" : "#5B7A9A", border: "none", borderRadius: 10, padding: "12px 28px", fontSize: 15, fontWeight: 700, cursor: selected !== null ? "pointer" : "default" }}>
              ✔ Valider
            </button>
          ) : (
            <button onClick={handleNext}
              style={{ background: "#1A6FA8", color: "#fff", border: "none", borderRadius: 10, padding: "12px 28px", fontSize: 15, fontWeight: 700, cursor: "pointer" }}>
        ...
        <div style={{ background: "#0F2A45", borderRadius: 20, padding: "32px 36px", boxShadow: "0 20px 60px #0008", border: "1px solid #1A4A7A" }}>
          {!teacherMode ? (
            <>
              ...
              <p style={{ color: "#5B7A9A", fontSize: 13, marginBottom: 24 }}>
                {QUESTIONS.length} questions · 3 niveaux · résultats envoyés à l'enseignant
              </p>
              ...
              <button onClick={() => onStudent(name.trim())} disabled={name.trim().length < 3}
                style={{ width: "100%", background: name.trim().length >= 3 ? "linear-gradient(90deg,#1A6FA8,#00B4D8)" : "#1A3A5A", color: name.trim().length >= 3 ? "#fff" : "#3A6A8A", border: "none", borderRadius: 10, padding: 14, fontSize: 16, fontWeight: 800, cursor: name.trim().length >= 3 ? "pointer" : "default", transition: "all 0.2s" }}>
                🚀 Démarrer le QCM
              </button>
              <button onClick={() => setTeacherMode(true)}
                style={{ width: "100%", background: "none", color: "#3A6A8A", border: "1px solid #1A3A5A", borderRadius: 10, padding: 10, fontSize: 13, cursor: "pointer", marginTop: 12 }}>
                🔐 Accès enseignant
              </button>
            </>
          ) : (
            <>
              ...
              <input type="password" value={pwd}
                ...
                placeholder="••••••••"
                style={{ width: "100%", padding: "12px 14px", borderRadius: 10, border: `2px solid ${pwdError ? "#E74C3C" : "#1A4A7A"}`, background: "#0A1A2A", color: "#E8F4FB", fontSize: 15, outline: "none", boxSizing: "border-box", fontFamily: "inherit" }}
              />
              {pwdError && <p style={{ color: "#E74C3C", fontSize: 12, margin: "4px 0 8px" }}>Mot de passe incorrect</p>}
              <p style={{ color: "#3A6A8A", fontSize: 11, marginBottom: 20 }}>Indice : iset + année en cours</p>
              <button onClick={handleTeacher}
                style={{ width: "100%", background: "linear-gradient(90deg,#F0A500,#E67E22)", color: "#0F2A45", border: "none", borderRadius: 10, padding: 14, fontSize: 16, fontWeight: 800, cursor: "pointer" }}>
                📊 Ouvrir le tableau de bord
              </button>
              <button onClick={() => { setTeacherMode(false); setPwd(""); setPwdError(false); }}
                style={{ width: "100%", background: "none", color: "#3A6A8A", border: "1px solid #1A3A5A", borderRadius: 10, padding: 10, fontSize: 13, cursor: "pointer", marginTop: 10 }}>
                ← Retour
              </button>
            </>
          )}
        </div>
        <p style={{ textAlign: "center", color: "#2A4A6A", fontSize: 11, marginTop: 16 }}>
          Résultats synchronisés en temps réel via Firebase · GitHub Pages
        </p>
      </div>
    </div>
  );
